...
curriculum_schema = {
    ...
    "lag": stdict(curriculum_base_schema),
}
...
training_schema = {
    ...
    "data": merge(tstring, allowed(["gaussian", "multi_context_mixture"])), 
    ...
    "resume_id": merge(tstring, nullable, default(None)),  # run folder UUID (where state.pt lives)
    "wandb_resume_id": merge(tstring, nullable, default(None)),  # wandb run id (e.g. q251a116) to resume same run
